...
void third_grid(Triangulation<3> &triangulation)
{

  const unsigned int number_of_cells = 10;
  const unsigned int number_of_rotations = 2;
  const double inner_radius = 3;
  const double cylinder_radius = 2;

  GridGenerator::moebius(triangulation, number_of_cells, number_of_rotations, inner_radius, cylinder_radius);
  
  triangulation.refine_global(3);
}

void fourth_grid(Triangulation<3> &triangulation)
{
  const double center_radius = 3;
  const double inner_radius = 2;

  GridGenerator::torus(triangulation, center_radius, inner_radius);
  triangulation.refine_global(2);
}

void distribute_dofs_3(DoFHandler<3> &dof_handler)
{
  const FE_Q<3> finite_element(1);
  dof_handler.distribute_dofs(finite_element);
  ...
  // With this, we can now write the results to a file:
  std::ofstream out("sparsity_pattern3.svg");
  sparsity_pattern.print_svg(out);
  // The result is stored in an <code>.svg</code> file, where each nonzero entry
  // in the matrix corresponds with a red square in the image. The output will
  // be shown below.
...
void renumber_dofs_3(DoFHandler<3> &dof_handler)
{
  DoFRenumbering::Cuthill_McKee(dof_handler);
  ...
  std::ofstream out("sparsity_pattern4.svg");
  sparsity_pattern.print_svg(out);
}
...
int main()
{
  ...
  Triangulation<3> second_triangulation;
  fourth_grid(second_triangulation);

  DoFHandler<3> second_dof_handler(second_triangulation);

  distribute_dofs_3(second_dof_handler);
  renumber_dofs_3(second_dof_handler);
}
