...
void Step6<dim>::solve()
{
  ...
  std::cout << "time: " << timer.cpu_time() << " s" << std::endl;
...
void Step6<dim>::output_results(const unsigned int cycle) const
{
  ...
  Vector<double> center_output;

  Point<3> p(0.5, 0.5, 0.5);

  VectorTools::point_value(dof_handler, solution, p, center_output);

  std::cout << "GREP" << cycle << ": " << alpha << " " << center_output(0) << std::endl;
}
...
int main()
{
  try
    {
      double alpha_min = 40.35;
      double alpha_max = 40.4;
      double alpha;               // -mu+nu
      double divisions = 200;
