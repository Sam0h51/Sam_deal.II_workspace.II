...
namespace Step26
{
                                     ...
                                     const unsigned int component) const
  {
    ...
    if(component == 0){
      ...
      return 2*std::sqrt(3)*(std::rand()%1001)/1000 - std::sqrt(3);
  ...
  void HeatEquation<dim, spacedim>::output_results() const
  {
    ...
    const std::string filename = 
      "timestep-convergence-test-" + Utilities::int_to_string(iteration_number, 3) + "-" + 
  ...
  void HeatEquation<dim, spacedim>::run()
  {
    ...
    // GridGenerator::torus(triangulation, 20., 10.);
    GridGenerator::hyper_cube(triangulation, -36, 36);
 ...
 */
  ...
  /// @brief This is a function I used to test the output of my Mass and Laplace matrices. This code is no longer used
  /// @tparam dim This is a template parameter, and gives the dimension of the manifold. This parameter is not
  ///             used in this portion of the code, but is required because test_matrices is contained in the
  ///             step-26 class, which uses both template arguments
  /// @tparam spacedim This is a template parameter, and gives the dimension of the embedding space. We use this to
  ///                  to create an InitialConditions function object.
  template <int dim, int spacedim>
...
int main()
{
  /* try
    {
      ...
      HeatEquation<2, 3> heat_equation_solver(1, 10, 6, 0.3, 0.0, 1., 80.);
  ...
  for(int i = 0; i < 10; ++i){
    try
    {
      ...
      const double g_constant = 0 /*0.2*i*/;

      const unsigned int timestep_denominator = 100 + 100*i;

      std::cout << "Running simulation with g_1 = " << g_constant << std::endl << std::endl;

      HeatEquation<2, 3> heat_equation_solver(1, timestep_denominator, 5, i+1, 0.3, g_constant, 1., 80.);
      heat_equation_solver.run();
    }
    catch (std::exception &exc)
      {
