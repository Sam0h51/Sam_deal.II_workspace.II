...
namespace Step26
{
                                   ...
                                   const unsigned int component) const
  {
    ...
    if(point_within_period <= .125 || point_within_period >= .875){
      if(std::abs(p(second_coordinate) - (std::tan(theta)*(p(0)))) <= 1e-1 &&
         p(0) >= 0){
        return 20;
      }
      else if(std::abs(p(second_coordinate) - (std::tan(theta)*(p(0)))) <= 1e-1 &&
         p(0) <= 0){
        return -20;
      }
      else{
      ...
      if(std::abs(p(0) - ((std::cos(theta)/std::sin(theta))*(p(second_coordinate)))) <= 1e-1
         && p(second_coordinate) >= 0){
        return 20;
      }
      else if(std::abs(p(0) - ((std::cos(theta)/std::sin(theta))*(p(second_coordinate)))) <= 1e-1
         && p(second_coordinate) <= 0){
        return -20;
      }
      else{
    ...
    /* if(point_within_period <= .25)
    {
      if ((1.5*std::sin(2*3.14159265358*point_within_period) <= p(0)) &&
        ...
        (1.5*std::cos(2*3.14159265358*point_within_period) <= p(second_coordinate)) &&
        (4.5*std::cos(2*3.14159265358*point_within_period) >= p(second_coordinate))){
          return 1;
        }
      else
        return -.05;
...
template<int dim, int spacedim>
void HeatEquation<dim, spacedim>::hyper_cube()
{
  ...
  try
    {
      ...
      HeatEquation<2, 2> heat_equation_solver;
